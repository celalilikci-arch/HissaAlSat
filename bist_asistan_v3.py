...
# Sidebar değerleri yukarıda with st.sidebar bloğunda tanımlandı — burada override yok
    ...
    if alis_modu == "💰 Param Var → Kaç Lot?":
        ...
        if st.button("📊 Alış Planı Oluştur", type="primary", key="btn1"):
            ...
            for i in range(kademe_sayisi):
                ...
                toplam_lot += lot; toplam_maliyet += gercek
            ...
            st.success(f"✅ **{toplam_lot} Lot** | Kalan: **{para_fmt(butce-toplam_maliyet)} TL**")
            log_ekle("ALIŞ→LOT", hisse, f"{para_fmt(butce)} TL → {toplam_lot} lot")
        ...
        if st.button("💵 Gereken Nakdi Hesapla", type="primary", key="btn2"):
            ...
            st.dataframe(pd.DataFrame(plan), use_container_width=True, hide_index=True)
            st.success(f"✅ **{hedef_lot} Lot** için gerekli nakit: **{para_fmt(toplam_para)} TL**")
            log_ekle("LOT→NAKİT", hisse, f"{hedef_lot} lot → {para_fmt(toplam_para)} TL")
        ...
        if st.button("🔢 Paçal Denklemini Çöz", type="primary", key="btn3"):
            ...
            if hedef_maliyet >= mevcut_maliyet:
                ...
                st.error(f"Hedefe ulaşılamaz. Min. fiyat: {round(maliyet_lot,2)} TL")
            else:
                ...
                log_ekle("PAÇAL", hisse, f"Hedef {hedef_maliyet}₺ → {gerekli_lot} lot")
...
with tab_satis:
    ...
    if satis_modu == "💵 Ne Kadar Nakit → Kaç Lot?":
        ...
        if st.button("📊 Satış Planı", type="primary", key="btn4"):
            ...
            toplam_agirlik = sum(agirliklar)
            plan, toplam_lot, toplam_nakit, kalan_hedef = [], 0, 0, ihtiyac_nakit
            ...
            st.success(f"Hesabınıza geçecek: **{para_fmt(toplam_nakit)} TL**")
            log_ekle("SATIŞ→LOT", hisse_sat, f"Hedef {para_fmt(ihtiyac_nakit)} TL → {toplam_lot} lot")
        ...
        if st.button("💵 Ele Geçecek Tutarı Hesapla", type="primary", key="btn5"):
            ...
            st.dataframe(pd.DataFrame(plan), use_container_width=True, hide_index=True)
            st.success(f"**{satilacak_lot} Lot** → **{para_fmt(toplam_nakit)} TL** net")
            log_ekle("LOT→SATIŞ", hisse_sat, f"{satilacak_lot} lot → {para_fmt(toplam_nakit)} TL")
...
with tab_portfoy:
    ...
    secilen_hisseler = st.multiselect("Portföye Hisse Ekle", options=HISSE_KODLARI)

    # Seçilen hisselerin bilgilerini göster
    if secilen_hisseler:
        for kod in secilen_hisseler:
            if kod in BIST_HISSELER:
                st.markdown(f"**{kod}** — {BIST_HISSELER[kod]['firma']} *(Sektör: {BIST_HISSELER[kod]['sektor']})*")
    ...
    if secilen_hisseler:
        sektorler = [BIST_HISSELER[h]["sektor"] for h in secilen_hisseler if h in BIST_HISSELER]
        gorulmus = set()
        cakisan = set(x for x in sektorler if x in gorulmus or gorulmus.add(x))

        if cakisan:
            st.error(f"⚠️ Aynı sektörden birden fazla hisse var: **{chr(44).join(cakisan)}**")
        else:
            ...
            if toplam_oran != 100:
                ...
                if st.button("🧾 Alım Reçetesi Oluştur", type="primary"):
                    ...
                    for hisse, oran in oranlar.items():
                        ...
                        recete.append({"Hisse": hisse, "Sektör": sektor,
                            "Bütçe ₺": round(h_butce,2), "Lot*": lot, "Harcama ₺": round(harcama,2)})
                    ...
                    st.info(f"Kalan nakit: **{para_fmt(toplam_butce-toplam_h)} TL**")
                    st.write(f"🟢 Kâr-Al hedefi: **{para_fmt(toplam_h*(1+hedef_kar/100))} TL**")
                    st.write(f"🔴 Zarar-Kes sınırı: **{para_fmt(toplam_h*(1-zarar_kes/100))} TL**")
                    log_ekle("PORTFÖY", f"{len(secilen_hisseler)} hisse", f"Toplam: {para_fmt(toplam_h)} TL")

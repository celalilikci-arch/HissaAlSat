...
komisyon_orani_default = 2.0 / 1000
bsmv_orani_default = 5.0 / 100
efektif_komisyon = komisyon_orani_default * (1 + bsmv_orani_default)
    ...
    if alis_modu == "💰 Param Var → Kaç Lot?":
        ...
        if st.button("🔢 Paçal Denklemini Çöz", type="primary", key="btn3"):
            ...
            if hedef_maliyet >= mevcut_maliyet:
                ...
                st.info(f"Gerekli yatırım: **{round(gerekli_lot*maliyet_lot,2)} TL**")
...
with tab_satis:
    ...
    if satis_modu == "💵 Ne Kadar Nakit → Kaç Lot?":
        ...
        if st.button("💵 Ele Geçecek Tutarı Hesapla", type="primary", key="btn5"):
            ...
            log_ekle("LOT→SATIŞ", hisse_sat, f"{satilacak_lot} lot → {round(toplam_nakit,2)} TL")
        ...
        if st.button("📊 Reel Getiriyi Hesapla", type="primary", key="btn6"):
            ...
            if gun < 0:
                ...
                log_ekle("REEL KAR", "-", f"{gun} gün → Reel: {round(reel,2)} TL")
